...
func (mdwrk *Mdwrk) SendToBroker(command string, option string, msg []string) (err error) {
	...
	if mdwrk.verbose {
		logger.Infof("I: sending %s to broker %q\n", MDPS_COMMANDS[command], m)
	}
...
func (mdwrk *Mdwrk) ConnectToBroker() (err error) {
	...
	if mdwrk.verbose {
		logger.Infof("I: connecting to broker at %s...\n", mdwrk.broker)
	}
...
func (mdwrk *Mdwrk) Recv(reply []string) (msg []string, err error) {
	...
	for {
		...
		if len(polled) > 0 {
			...
			if mdwrk.verbose {
				logger.Infof("I: received message from broker: %q\n", msg)
			}
			mdwrk.liveness = heartbeat_liveness
			...
			default:
				logger.Errorf("E: invalid input message %q\n", msg)
			}
		} else {
			mdwrk.liveness--
			if mdwrk.liveness == 0 {
				if mdwrk.verbose {
					logger.Warnf("W: disconnected from broker - retrying...")
				}
