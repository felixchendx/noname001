...
func (mdcli *Mdcli) ConnectToBroker() (err error) {
	...
	if err != nil {
		if mdcli.verbose {
			logger.Errorf("E: ConnectToBroker() creating socket failed")
		}
		return
	}
	...
	if mdcli.verbose {
		logger.Infof("I: connecting to broker at %s...", mdcli.broker)
	}
	err = mdcli.client.Connect(mdcli.broker)
	if err != nil && mdcli.verbose {
		logger.Errorf("E: ConnectToBroker() failed to connect to broker", mdcli.broker)
	}
...
func (mdcli *Mdcli) Send(service string, request ...string) (reply []string, err error) {
	...
	req[0] = MDPC_CLIENT
	if mdcli.verbose {
		logger.Infof("I: send request to '%s' service: %q\n", service, req)
	}
	for retries_left := mdcli.retries; retries_left > 0; retries_left-- {
		...
		if len(polled) > 0 {
			...
			if mdcli.verbose {
				logger.Infof("I: received reply: %q\n", msg)
			}
			...
			if mdcli.verbose {
				logger.Warnf("W: no reply, reconnecting...")
			}
	...
	if mdcli.verbose {
		logger.Warnf("W: permanent error, abandoning")
	}
	return
}
