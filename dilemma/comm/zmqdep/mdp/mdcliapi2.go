...
func (mdcli2 *Mdcli2) ConnectToBroker() (err error) {
	...
	if mdcli2.verbose {
		logger.Infof("I: connecting to broker at %s...", mdcli2.broker)
	}
	err = mdcli2.client.Connect(mdcli2.broker)
	if err != nil && mdcli2.verbose {
		logger.Errorf("E: ConnectToBroker() failed to connect to broker", mdcli2.broker)
	}
...
func (mdcli2 *Mdcli2) Recv() (msg []string, err error) {
	...
	if len(polled) > 0 {
		...
		if err != nil {
			logger.Warnf("W: interrupt received, killing client...")
			return
		}
