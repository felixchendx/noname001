...
func NewDevice(params *DahuaDeviceParams) (*DahuaDevice, error) {
	var err error

	dev := &DahuaDevice{}
	dev.context, dev.cancel = context.WithCancel(params.ParentContext)
	dev.logger, dev.logPrefix = params.Logger, params.LogPrefix + ".dah"
	...
	dev.newCache()
	dev.cache.lDat.State = baseTyping.DEVICE_LIVE_STATE__NEW

	return dev, nil
}

type DahuaDevicePatchParams struct {
	Name  string
	State string
	Brand string
...
// iffy
func (dev *DahuaDevice) PatchAndReload(patchParams *DahuaDevicePatchParams) (error) {
	var err error
