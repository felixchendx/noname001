...
type DeviceProviderParams struct {
...
func NewDeviceProvider(params *DeviceProviderParams) (*DeviceProvider, error) {
	var err error
	
	pvdr := &DeviceProvider{}
	pvdr.context, pvdr.cancel = context.WithCancel(params.Context)
	pvdr.logger = params.Logger
	pvdr.logPrefix = params.LogPrefix + ".dvc.pvdr"

	pvdr.tempNodeID = params.TempNodeID
	pvdr.providerID = DeviceProviderID(pvdr.tempNodeID)
...
func (pvdr *DeviceProvider) AssignDataHandler(dataHandler DataHandlerIntface) {
	pvdr.dataHandler = dataHandler
}

func (pvdr *DeviceProvider) Connect() (err error) {
	// TODO: idempotency
		...
		for {
			...
			for {
				...
				if len(request) > 0 {
					...
					case CMD_DEVICE_SNAPSHOT_LIST: reply[0] = pvdr.recvRequest_DeviceSnapshotList(reqParam)
					case CMD_DEVICE_SNAPSHOT     : reply[0] = pvdr.recvRequest_DeviceSnapshot(reqParam)
					default:
...
func (pvdr *DeviceProvider) Disconnect() (err error) {
	return
}

// DO NOT USE, will cause panic
// func (sp *DeviceProvider) Stop() {
// 	if sp.MDWorker != nil {
// 		sp.MDWorker.Close()
